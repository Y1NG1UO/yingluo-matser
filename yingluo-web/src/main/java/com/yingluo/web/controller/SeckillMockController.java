...
import static com.yingluo.core.enums.SeckillSolutionEnum.*;
...
public class SeckillMockController {
    ...
    public Result doWithKafkaMqMessage(@RequestBody @Valid SeckillWebMockRequestDTO dto) {
        processSeckill(dto, KAFKA_MQ, () -> {
            ...
            kafkaTemplate.send("yingluo-kafka", phoneNumber, JSON.toJSONString(payload));
        });
        return Result.ok();
        //待mq监听器处理完成打印日志，不在此处打印日志
    }
    ...
    /**
     * canal测试方法说明：启动yingluo-canal模块下的CanalClientApplication类即可，canal使用默认配置。注意要先启动canal-server，并使用tcp模式
     * 秒杀结束后会在控台输出日志
     *
     * @param dto 秒杀活动
     * @see com.yingluo.web.stream.consumer.SeckillMockCanalResponseListener 为对应的消息接受者
     */
